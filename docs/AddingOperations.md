...
+ adding an entry to `opfunc_mapping` in [opfuncs.py](./opfuncs.py)
+ adding an entry to `_meta_ops` in [fake_ops.py](./fake_ops.py)
...
method to `OpOverrides` in [opoverrides.py](./opoverrides.py).
...
loop level IR in [lowering.py](./lowering.py) using the `@lowering.register_lowering`
decorator.

### Spyre-specific decompositions

We define Spyre-specific decompositions in [decompositions.py](./decompositions.py)
...
[customops.py](./customops.py). This has two pieces:
...
+ register a lowering for the custom op (eg `aten.mm.default`) and add method to `OpOverrides` in [opoverrides.py](./opoverrides.py).
