...
This repo will hold the required Pytorch C++ and Python layer code for supporting the IBM Spyre device.
...
* this one: torch-2.7.1+cpu.cxx11.abi-cp311-cp311-linux_x86_64.whl
...
`torch_spyre`: This will contain all required Python code to enable eager (currently this is being updated and much of it will be removed). Please refer to https://github.com/pytorch/pytorch/tree/v2.7.1/test/cpp_extensions/open_registration_extension for any of the current Python files. For the most part, all that will be necessary from a Python standpoint is registering the device with PrivateUse1
